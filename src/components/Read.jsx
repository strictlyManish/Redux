  ...
  return (
    <div className="bg-gray-700 px-12 py-12 w-2/4 rounded">
      {tasks.length > 0 ? tasks.map((obj)=>{
        return(
          <li key={obj.id}
            className="bg-gray-800 px-2 py-2 text-gray-500 list-decimal rounded mt-1"
          >
            <p className="uppercase font-bold">{obj.title}</p>
            <p className="mt-1">{obj.content}</p>
            <button onClick={()=>dispatch(removeTask(obj.id))} className="mt-2 active:text-white cursor-pointer">Remove</button>
          </li>
        )
